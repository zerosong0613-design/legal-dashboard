...
### v1.4 — P0 데이터 안정성·보안 (`fe4b493`)
- **데이터 충돌 해결**: updatedAt/version 메타데이터, 충돌 모달 (클라우드/로컬 선택)
- **API Key 난독화**: base64 역순 인코딩, 보안 경고, 키 삭제 버튼
...
### ✅ P0 — 핵심 (데이터 안정성·보안) — 완료 (`fe4b493`)

| 항목 | 상태 | 구현 내용 |
|------|------|------|
| **데이터 충돌 해결** | ✅ | updatedAt/version 기반 충돌 감지, 모달로 클라우드/로컬 선택 |
| **API Key 보안** | ✅ | base64 난독화 저장, 보안 경고, 키 삭제/수동 동기화 버튼 |
| **자동 백업** | ✅ | 5분 주기 + beforeunload 자동 저장, 3슬롯 로테이션 복원 |
| **인증 강화** | ✅ | SHA-256 PIN 해시 (Web Crypto API), 다중 사용자 지원 |
